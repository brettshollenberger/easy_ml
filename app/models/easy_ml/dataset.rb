...
module EasyML
  class Dataset < ActiveRecord::Base
    ...
    def datasource
      ...
      config = read_attribute(:datasource)
    ...
    # end
    #
    has_many :dataset_statistics, class_name: "EasyML::DatasetStatistics"
    ...
    dependency :raw do |dependency|
      dependency.option :file do |option|
        option.default
        option.set_class EasyML::Data::Splits::FileSplit
      ...
      dependency.option :memory do |option|
        option.set_class EasyML::Data::Splits::InMemorySplit
      end
    ...
    attribute :dataset_statistics_id
    def preprocessor
      return @preprocessor if @preprocessor.present?

      # If we're running inference, a model will pass the dataset a locked set of statistics to reuse
      @preprocessor = if dataset_statistics_id.present?
                        EasyML::DatasetStatistics.find(dataset_statistics_id)
                      else
                        EasyML::DatasetStatistics.new(
