...
module EasyML
  class DatasetStatistics < ActiveRecord::Base
    self.table_name = "easy_ml_dataset_statistics"
    ...
    def statistics
      deserialize_statistics(read_attribute(:statistics))
    end
    ...
    def initialize_imputers(config)
      standardize_config(config).each_with_object({}) do |(col, strategies), hash|
        ...
        strategies.each do |strategy, options|
          ...
          imputer_stats = (statistics || {}).dig(col.to_s)
